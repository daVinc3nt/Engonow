...
import { Fill_Question } from "../../ListeningInterface";
...
export default function FQuestion({ question, answer, setAnswer }: Props) {
	...
	return (
		<div
			...
			className="w-full h-fit p-1 grid grid-cols-12 gap-2 group">
			...
			<div
				...
				<input
					...
					className="w-full h-8 border-2 border-red-200 p-2 rounded-lg outline-none 
					focus:border-transparent focus:outline focus:ring focus:ring-red-400  
					group-target:ring group-target:ring-red-400
					bg-primary focus:bg-white active:bg-white"
				/>
			</div>
		</div>
	);
